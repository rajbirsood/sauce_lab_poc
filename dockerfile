...
COPY sc-4.5.0-linux /sc-4.5.0-linux

#Configure Sauce connect binary
RUN ["cp", "-p", "sc-4.5.0-linux/bin/sc", "/usr/local/bin/sc"]
...
RUN ["cp", "-p", "sc-4.5.0-linux/config_examples/systemd/sc.service",
"/etc/systemd/system/sc.service"]
RUN ["cp", "-p", "sc-4.5.0-linux/config_examples/systemd/sc@.service",
"/etc/systemd/system/sc@.service"]
...
RUN ["ln", "-s", "/etc/systemd/system/sc@.service",
"./sc.service.wants/sc@8000.service"]
RUN ["ln", "-s", "/etc/systemd/system/sc@.service",
"./sc.service.wants/sc@8001.service"]
